...
int is_filtering_active(const Config *cfg) {
    return cfg->git_only || cfg->grep_pattern || cfg->min_size > 0 || cfg->dir_only;
}
...
void print_entry(const FileEntry *fe, int depth, int was_expanded, int has_visible_children, const PrintContext *ctx) {
    ...
    if (is_dir && fe->is_git_root) {
        ...
        if (git_get_branch_info(fe->path, &gi)) {
            ...
            if (gi.commit[0]) {
                EMIT(line, pos, ENTRY_BUF_SIZE, " %s%s%s%s", CLR(ctx->cfg, COLOR_GREY), CLR(ctx->cfg, STYLE_ITALIC), gi.commit, RST(ctx->cfg));
            }
            if (fe->tag) {
                EMIT(line, pos, ENTRY_BUF_SIZE, " %s(%s)%s", CLR(ctx->cfg, COLOR_GREY), fe->tag, RST(ctx->cfg));
            }
            if (gi.has_upstream) {
